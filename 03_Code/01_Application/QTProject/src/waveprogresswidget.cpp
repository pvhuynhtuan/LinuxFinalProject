...
WaveProgressWidget::WaveProgressWidget(QWidget *parent)
    : QWidget(parent),
    m_value(0),
    m_minValue(0),
    m_maxValue(100),
...
WaveProgressWidget::~WaveProgressWidget()
{
    if(m_timer->isActive())
    {
        m_timer->stop();
...
void WaveProgressWidget::setRange(int minValue, int maxValue)
{
...
void WaveProgressWidget::setMinValue(int minValue)
{
    setRange(minValue, m_maxValue);
}

void WaveProgressWidget::setMaxValue(int maxValue)
{
    setRange(m_minValue, maxValue);
}

void WaveProgressWidget::setValue(int value)
{
    if(m_value != value)
    {
        m_value = value;
...
void WaveProgressWidget::setWaterDensity(int waterDensity)
{
...
void WaveProgressWidget::setWaterHeight(double waterHeight)
{
...
void WaveProgressWidget::setOffset(int offset)
{
    if(m_offset != offset)
    {
        m_offset = offset;
...
void WaveProgressWidget::setBorderWidth(double borderWidth)
{
...
void WaveProgressWidget::setBackgroundColor(const QColor &backgroundColor)
{
...
void WaveProgressWidget::setPointerStyle(const PercentStyle &percentStyle)
{
...
QSize WaveProgressWidget::sizeHint() const
{
    return QSize(200, 200);
}

void WaveProgressWidget::timeout()
{
    m_offset += 0.4;
    update();
}

void WaveProgressWidget::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
    ...
    const int textWidth = width() / 2 - 25;
...
void WaveProgressWidget::drawValue(QPainter *painter)
{
    const int h = height();
    const int w = width();
